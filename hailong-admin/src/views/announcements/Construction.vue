...
const loadData = async () => {
  ...
  try {
    ...
    const res = await constructionApi.getAnnouncements(params)
...
const handleEdit = async (row) => {
  isEdit.value = true
  try {
    const res = await constructionApi.getAnnouncement(row.id)
    if (res.success && res.data) {
      // 映射后端DTO字段到前端表单
      Object.assign(formData, {
        id: res.data.id,
        ...
        winner: res.data.winner || '', // 数据库字段winner对应中标人
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await constructionApi.deleteAnnouncement(row.id)
    if (res.success) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    let res
    if (isEdit.value) {
      res = await constructionApi.updateAnnouncement(formData.id, submitData)
    } else {
      res = await constructionApi.createAnnouncement(submitData)
    }
